...
CORE_MOTION_CONFLICT_FALLBACK_ANGLE_DEG = 35.0
CORE_MOTION_MIN_DIRECTION_PIXELS = 30
CORE_MOTION_LATEST_STEP_MAX_DEVIATION_DEG = 60.0
...
def robust_core_vector(track):
    ...
    # Reject a sudden last-frame core jump that contradicts the established
    # earlier core trend. Strong radar pixels can rapidly shrink, split or
    # move inside one rain cell even when the cell itself keeps moving in
    # the same direction. Such an intensity jump must not reverse the arrow.
    if len(track) >= 4:
        previous_index, previous = track[-2]
        history_gap = previous_index - first_index
        latest_gap = last_index - previous_index
        if history_gap > 0 and latest_gap > 0:
            history_dx = (previous["cx"] - first["cx"]) / history_gap
            history_dy = (previous["cy"] - first["cy"]) / history_gap
            latest_dx = (last["cx"] - previous["cx"]) / latest_gap
            latest_dy = (last["cy"] - previous["cy"]) / latest_gap
            history_mag = math.hypot(history_dx, history_dy)
            latest_mag = math.hypot(latest_dx, latest_dy)
            if (
                history_mag >= CORE_MOTION_MIN_MAG
                and latest_mag >= CORE_MOTION_MIN_MAG
            ):
                history_angle = (
                    math.degrees(math.atan2(history_dy, history_dx)) + 360.0
                ) % 360.0
                latest_angle = (
                    math.degrees(math.atan2(latest_dy, latest_dx)) + 360.0
                ) % 360.0
                if (
                    angle_difference_deg(history_angle, latest_angle)
                    > CORE_MOTION_LATEST_STEP_MAX_DEVIATION_DEG
                ):
                    return None
...
def merge_motion_signals(precip_motion, core_motion, precip_component):
    """Combine motion cues conservatively and reject anomalous core reversals."""
    ...
    # A tiny black core is still useful for DISPLAY, but its centroid can jump
    # a lot when only a few strongest radar pixels brighten/fade. Do not let
    # such a small core determine the arrow by itself.
    if core_motion["core_area"] < CORE_MOTION_MIN_DIRECTION_PIXELS:
        if precip_motion is None:
            return None
        out = dict(precip_motion)
        out["motion_source"] = "precipitation"
    ...
    if precip_motion is not None:
        ...
        if direction_difference <= CORE_MOTION_BLEND_MAX_ANGLE_DEG:
            # Both independent methods agree: keep the strong core dominant,
            # with the larger rain field damping small centroid jitter.
            vx = (
        ...
        elif direction_difference > CORE_MOTION_CONFLICT_FALLBACK_ANGLE_DEG:
            # Strong disagreement means the intense core most likely changed
            # shape/identity inside the cloud. Prefer the independently stable
            # full precipitation motion instead of allowing a sudden false turn.
            out = dict(precip_motion)
            out["motion_source"] = "precipitation"
            return out
