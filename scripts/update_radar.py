...
# Display-only outer-contour tightening of red precipitation areas.
# Enclosed internal holes are preserved and are not enlarged by this erosion.
# This does NOT affect precipitation detection, motion tracking, or core logic.
DISPLAY_PRECIP_EROSION_PIXELS = 1

# Display-only repair of small compact enclosed holes. SHMU/static-color
# quantization can occasionally leave regular white blocks inside a broad red
# precipitation area. Only small, fully enclosed and reasonably compact holes
# are filled; gaps connected to the exterior and larger real dry slots remain.
# Analytical precipitation masks used by every motion algorithm stay unchanged.
DISPLAY_FILL_ENCLOSED_HOLES = True
DISPLAY_HOLE_MAX_PIXELS = 144
DISPLAY_HOLE_MAX_WIDTH = 16
DISPLAY_HOLE_MAX_HEIGHT = 16
DISPLAY_HOLE_MIN_FILL_RATIO = 0.45
...
# Cross-check strong-core motion against the independent precipitation-field
# motion. A core is still preferred when both signals agree. Moderate conflicts
# are accepted only with a full/strong core history; hard contradictions never
# let the core override a reliable precipitation vector.
CORE_MOTION_CONFLICT_MAX_ANGLE_DEG = 55.0
CORE_MOTION_CONFLICT_MIN_POSITIONS = 5
CORE_MOTION_CONFLICT_MIN_SAMPLES = 6

# One-frame direction jump guard. It compares an arrow with the previous
# 5-minute radar result only when the same component can be matched
# unambiguously. A turn >60 degrees must have an independent/strong internal
# confirmation; otherwise that one arrow is suppressed for the frame.
TEMPORAL_TURN_GUARD_ENABLED = True
TEMPORAL_TURN_MAX_ANGLE_DEG = 60.0
TEMPORAL_TURN_GUARD_MAX_GAP_MINUTES = 6.0
TEMPORAL_COMPONENT_MATCH_MAX_DISTANCE_PX = 60.0
TEMPORAL_COMPONENT_MATCH_MAX_AREA_RATIO = 2.25
TEMPORAL_COMPONENT_MATCH_AMBIGUITY_MARGIN = 8.0
TEMPORAL_PRECIP_MIN_SAMPLES_FOR_TURN = 3
...
# Final large-front animation fallback. This is deliberately BELOW all existing
# component/core/front-fallback logic and therefore cannot replace an arrow that
# the current system already knows how to calculate. It is used only for a very
# large unresolved precipitation system. Instead of correlating one binary blob,
# it tracks the internal RGB radar texture in several overlapping spatial tiles
# and across several consecutive frames, then requires temporal + spatial
# direction consensus and at least one independent broad-front confirmation.
LARGE_FRONT_ANIMATION_ENABLED = True
LARGE_FRONT_MIN_PIXELS = 12000
LARGE_FRONT_MIN_MAP_SHARE = 0.10
LARGE_FRONT_HISTORY_FRAMES = 6       # latest ~25 min
LARGE_FRONT_ROI_MARGIN = 48
LARGE_FRONT_TILE_SIZE = 96
LARGE_FRONT_TILE_STEP = 48
LARGE_FRONT_MIN_TILE_PIXELS = 400
LARGE_FRONT_MIN_GLOBAL_PIXELS = 1200
LARGE_FRONT_MIN_PHASE_CONFIDENCE = 1.06
LARGE_FRONT_MIN_VALID_VECTORS = 5
LARGE_FRONT_MIN_FRAME_PAIRS = 3
LARGE_FRONT_MIN_SPATIAL_TILES = 3
LARGE_FRONT_DIRECTION_CONSENSUS = 0.65
LARGE_FRONT_MAX_DIRECTION_DEVIATION_DEG = 30.0
LARGE_FRONT_CONFIRMATION_MAX_ANGLE_DEG = 50.0
LARGE_FRONT_HARD_CONTRADICTION_ANGLE_DEG = 90.0
LARGE_FRONT_MIN_MOTION_MAG = 0.6
LARGE_FRONT_MAX_SHIFT_PER_FRAME = 35.0
...
def exterior_background_mask(mask: np.ndarray, padding: int = 1) -> np.ndarray:
    """Return background connected to the image exterior; enclosed holes stay False."""
    ...
    # The artificial padding is guaranteed to be outside the precipitation.
    q = deque([(0, 0)])
    exterior[0, 0] = True
    ...
    while q:
        ...
        for ny in range(y0, y1):
            for nx in range(x0, x1):
                if ny == y and nx == x:
                    continue
                if background[ny, nx] and not exterior[ny, nx]:
                    exterior[ny, nx] = True
                    q.append((ny, nx))
...
def erode_outer_contour_only(mask: np.ndarray, pixels: int) -> np.ndarray:
    """Shrink only the external precipitation contour without enlarging enclosed holes."""
    if pixels <= 0 or not np.any(mask):
        return mask.copy()

    pad = max(1, int(pixels))
    padded_mask = np.pad(mask.astype(bool), pad, mode="constant", constant_values=False)

    # Only background connected to the outside may eat into the precipitation.
    # Background trapped inside a rain area is intentionally excluded, so an
    # existing white hole keeps its original size instead of growing by erosion.
    exterior = exterior_background_mask(mask, padding=pad)
    size = pixels * 2 + 1
    exterior_img = Image.fromarray((exterior.astype(np.uint8) * 255), mode="L")
    near_exterior = np.array(
        exterior_img.filter(ImageFilter.MaxFilter(size)),
        dtype=np.uint8,
    ) > 0

    trimmed = padded_mask & ~near_exterior
    return trimmed[pad:pad + mask.shape[0], pad:pad + mask.shape[1]]


def fill_small_enclosed_display_holes(mask: np.ndarray):
    """Fill only small compact holes completely enclosed by precipitation."""
    stats = {
        "detected": 0,
        "filled": 0,
        "filled_pixels": 0,
        "largest_filled_pixels": 0,
    }
    if not DISPLAY_FILL_ENCLOSED_HOLES or not np.any(mask):
        return mask.copy(), stats

    pad = 1
    exterior_padded = exterior_background_mask(mask, padding=pad)
    exterior = exterior_padded[pad:pad + mask.shape[0], pad:pad + mask.shape[1]]
    enclosed = (~mask.astype(bool)) & (~exterior)
    if not np.any(enclosed):
        return mask.copy(), stats

    h, w = enclosed.shape
    visited = np.zeros(enclosed.shape, dtype=bool)
        ...
        while stack:
            ...
            for ny in range(max(0, y - 1), min(h - 1, y + 1) + 1):
                for nx in range(max(0, x - 1), min(w - 1, x + 1) + 1):
                    if ny == y and nx == x:
                        continue
                    if enclosed[ny, nx] and not visited[ny, nx]:
                        visited[ny, nx] = True
                        stack.append((ny, nx))

        stats["detected"] += 1
        area = len(pixels)
        hole_w = max_x - min_x + 1
        hole_h = max_y - min_y + 1
        bbox_area = max(1, hole_w * hole_h)
        fill_ratio = area / bbox_area

        if (
            area <= DISPLAY_HOLE_MAX_PIXELS
            and hole_w <= DISPLAY_HOLE_MAX_WIDTH
            and hole_h <= DISPLAY_HOLE_MAX_HEIGHT
            and fill_ratio >= DISPLAY_HOLE_MIN_FILL_RATIO
        ):
            out.flat[np.array(pixels, dtype=np.int32)] = True
            stats["filled"] += 1
            stats["filled_pixels"] += area
            stats["largest_filled_pixels"] = max(
                stats["largest_filled_pixels"],
                area,
            )
...
def trim_precip_for_display(
    precip: np.ndarray,
    strong_core: Optional[np.ndarray] = None,
    return_stats: bool = False,
):
    """Tighten outer contour and repair only small enclosed display artifacts."""
    display_precip = erode_outer_contour_only(
        precip,
        DISPLAY_PRECIP_EROSION_PIXELS,
    )

    # This repair is display-only. The analytical mask passed to all motion
    # functions remains the original filtered precipitation mask.
    display_precip, hole_stats = fill_small_enclosed_display_holes(display_precip)

    # Never allow display-only processing to erase the black core support area.
    if strong_core is not None and np.any(strong_core):
        display_precip |= strong_core

    if return_stats:
        return display_precip, hole_stats
...
def compose_radar(
    ...
    # For display only, the external red precipitation contour is tightened by
    # 1 px. Enclosed internal white holes keep their original size.
...
def merge_motion_signals(precip_motion, core_motion, precip_component):
    """Cross-check core motion against precipitation motion before choosing an arrow."""
    ...
    if precip_motion is not None:
        ...
        if direction_difference <= CORE_MOTION_BLEND_MAX_ANGLE_DEG:
            # Best case: two independent signals agree. Keep the core dominant
            # and use the broad precipitation field to reduce centroid jitter.
            vx = (
        ...
        elif direction_difference <= CORE_MOTION_CONFLICT_MAX_ANGLE_DEG:
            # Moderate disagreement can happen when a strong core propagates
            # inside a broader rain area. Trust it only with a complete and
            # well-supported multi-frame core history. Otherwise prefer the
            # independently reliable precipitation vector.
            core_history_strong = (
                int(core_motion["positions"]) >= CORE_MOTION_CONFLICT_MIN_POSITIONS
                and int(core_motion["samples"]) >= CORE_MOTION_CONFLICT_MIN_SAMPLES
            )
            if core_history_strong:
                source = "strong_core_conflict_checked"
            else:
                vx = precip_vx
                vy = precip_vy
                source = "precipitation_core_conflict"
        else:
            # Hard contradiction. This is the failure mode that can create a
            # one-frame arrow pointing almost opposite to the rain field when
            # cores merge/split or the strongest spot changes identity.
            # Never allow the core to override a reliable precipitation vector.
            vx = precip_vx
            vy = precip_vy
            source = "precipitation_core_rejected"

    return {
        "vx": float(vx),
        "vy": float(vy),
        "center": (
            precip_motion["center"]
            if source.startswith("precipitation") and precip_motion is not None
            else core_motion["center"]
        ),
        "area": int(precip_component["area"]),
        "samples": int(
            precip_motion["samples"]
            if source.startswith("precipitation") and precip_motion is not None
            else core_motion["samples"]
        ),
...
def load_previous_motion_info(current_radar_dt: datetime):
    """Load only the immediately previous radar result for one-frame turn checks."""
    if not TEMPORAL_TURN_GUARD_ENABLED or not INFO_JSON.exists():
        return None
    try:
        previous = json.loads(INFO_JSON.read_text(encoding="utf-8"))
        previous_raw = previous.get("latest_radar_utc")
        if not previous_raw:
            return None
        previous_dt = datetime.fromisoformat(previous_raw.replace("Z", "+00:00"))
        if previous_dt.tzinfo is None:
            previous_dt = previous_dt.replace(tzinfo=timezone.utc)
        gap_minutes = (current_radar_dt - previous_dt).total_seconds() / 60.0
        if gap_minutes <= 0 or gap_minutes > TEMPORAL_TURN_GUARD_MAX_GAP_MINUTES:
            return None
        if not isinstance(previous.get("movement_components"), list):
            return None
        return previous
    except Exception:
        # Previous diagnostics must never be able to break radar generation.
        return None


def previous_component_match(motion, previous_info):
    """Find one unambiguous previous arrow belonging to the same rain component."""
    if previous_info is None:
        return None

    cx, cy = motion["center"]
    area = max(1, int(motion["area"]))
    scored = []
    for previous in previous_info.get("movement_components", []):
        center = previous.get("center_px") or {}
        vector = previous.get("movement_vector_px_per_5min") or {}
        if not all(k in center for k in ("x", "y")):
            continue
        if not all(k in vector for k in ("vx", "vy")):
            continue
        previous_area = max(1, int(previous.get("area_pixels", 1)))
        area_ratio = max(area, previous_area) / min(area, previous_area)
        if area_ratio > TEMPORAL_COMPONENT_MATCH_MAX_AREA_RATIO:
            continue
        distance = math.hypot(cx - float(center["x"]), cy - float(center["y"]))
        if distance > TEMPORAL_COMPONENT_MATCH_MAX_DISTANCE_PX:
            continue
        area_penalty = abs(math.log(area_ratio)) * 10.0
        scored.append((distance + area_penalty, distance, previous))
    ...
    if (
        len(scored) >= 2
        and scored[1][0] - scored[0][0] < TEMPORAL_COMPONENT_MATCH_AMBIGUITY_MARGIN
    ):
        return None
    return scored[0][2], scored[0][1]


def motion_has_turn_confirmation(motion) -> bool:
    """Return True when a sharp turn has enough independent/internal evidence."""
    source = motion.get("motion_source", "")
    if source == "strong_core_blend":
        return True
    if source.startswith("strong_core"):
        # Moderate core/precip conflicts are allowed only after the stronger
        # history gate in merge_motion_signals. If a precipitation vector also
        # exists within the allowed conflict angle, it confirms the turn.
        diff = motion.get("direction_difference_deg")
        return (
            motion.get("precip_vx") is not None
            and diff is not None
            and diff <= CORE_MOTION_CONFLICT_MAX_ANGLE_DEG
            and int(motion.get("core_positions", 0)) >= CORE_MOTION_CONFLICT_MIN_POSITIONS
            and int(motion.get("core_samples", 0)) >= CORE_MOTION_CONFLICT_MIN_SAMPLES
        )
    if source.startswith("precipitation"):
        # A precipitation arrow already represents several phase-correlation
        # measurements. Require one extra sample for a >60 degree turn.
        return int(motion.get("samples", 0)) >= TEMPORAL_PRECIP_MIN_SAMPLES_FOR_TURN
    if source == "front_fallback":
        return (
            int(motion.get("front_phase_samples", 0)) >= 2
            and len(motion.get("front_confirmations", [])) >= 1
        )
    if source == "large_front_animation":
        return (
            int(motion.get("animation_agreeing_vectors", 0))
            >= LARGE_FRONT_MIN_VALID_VECTORS
            and int(motion.get("animation_frame_pairs", 0))
            >= LARGE_FRONT_MIN_FRAME_PAIRS
            and int(motion.get("animation_spatial_tiles", 0))
            >= LARGE_FRONT_MIN_SPATIAL_TILES
            and len(motion.get("animation_confirmations", [])) >= 1
        )
    return False


def apply_temporal_turn_guard(motion, previous_info):
    """Suppress an unconfirmed one-frame direction flip of a matched component."""
    if motion is None or previous_info is None:
        return motion, False

    matched = previous_component_match(motion, previous_info)
    if matched is None:
        return motion, False

    previous, distance = matched
    previous_vector = previous.get("movement_vector_px_per_5min") or {}
    previous_vx = float(previous_vector["vx"])
    previous_vy = float(previous_vector["vy"])
    current_angle = (
        math.degrees(math.atan2(motion["vy"], motion["vx"])) + 360.0
    ) % 360.0
    previous_angle = (
        math.degrees(math.atan2(previous_vy, previous_vx)) + 360.0
    ) % 360.0
    turn = angle_difference_deg(current_angle, previous_angle)

    motion["temporal_previous_direction"] = direction_label(previous_vx, previous_vy)
    motion["temporal_turn_deg"] = float(turn)
    motion["temporal_match_distance_px"] = float(distance)

    if turn <= TEMPORAL_TURN_MAX_ANGLE_DEG:
        motion["temporal_guard_action"] = "accepted"
        return motion, False

    if motion_has_turn_confirmation(motion):
        motion["temporal_guard_action"] = "accepted_confirmed_turn"
        return motion, False

    # Do not freeze/reuse yesterday's vector: simply hide this one uncertain
    # arrow. If the new direction persists, the next frame can establish it.
    return None, True
...
def rgb_motion_features(rgb: np.ndarray, mask: np.ndarray) -> np.ndarray:
    """Build zero-background multi-channel radar texture features for phase correlation."""
    a = rgb.astype(np.float32) / 255.0
    r = a[:, :, 0]
    g = a[:, :, 1]
    b = a[:, :, 2]
    ...
    # Two chromatic axes preserve movement of SHMU intensity/color structures;
    # value adds a brightness cue. Static map content is zeroed by the already
    # filtered analytical precipitation mask.
    features = np.stack((r - g, g - b, value), axis=2).astype(np.float32)
    m = mask.astype(bool)
    if not np.any(m):
    ...
    for channel in range(features.shape[2]):
        ...
        mean = float(plane[m].mean()) if np.any(m) else 0.0
        plane = (plane - mean) * m
        features[:, :, channel] = plane

    h, w = mask.shape
    wy = np.hanning(h).astype(np.float32) if h > 2 else np.ones(h, dtype=np.float32)
    wx = np.hanning(w).astype(np.float32) if w > 2 else np.ones(w, dtype=np.float32)
    features *= (wy[:, None] * wx[None, :])[:, :, None]
    return features


def phase_shift_rgb_texture(
    a_rgb: np.ndarray,
    b_rgb: np.ndarray,
    a_mask: np.ndarray,
    b_mask: np.ndarray,
    frame_gap: int = 1,
    min_pixels: int = LARGE_FRONT_MIN_TILE_PIXELS,
    min_confidence: float = LARGE_FRONT_MIN_PHASE_CONFIDENCE,
):
    """Estimate RGB radar-texture movement, normalized to one 5-minute frame."""
    if (
        int(a_mask.sum()) < min_pixels
        or int(b_mask.sum()) < min_pixels
        or a_rgb.shape[:2] != a_mask.shape
        or b_rgb.shape[:2] != b_mask.shape
    ):
        return None

    a = rgb_motion_features(a_rgb, a_mask)
    b = rgb_motion_features(b_rgb, b_mask)
    if not np.any(a) or not np.any(b):
        return None

    fa = np.fft.fft2(a, axes=(0, 1))
    fb = np.fft.fft2(b, axes=(0, 1))
    cross = np.sum(fb * np.conj(fa), axis=2)
    cross /= np.maximum(np.abs(cross), 1e-9)
    ...
    exclusion = 2
    c2[
        max(0, peak_y - exclusion):min(h, peak_y + exclusion + 1),
        max(0, peak_x - exclusion):min(w, peak_x + exclusion + 1),
    ] = 0

    peak = float(corr[peak_y, peak_x])
    second = float(c2.max())
    confidence = peak / max(second, 1e-9)
    allowed = LARGE_FRONT_MAX_SHIFT_PER_FRAME * max(1, frame_gap)
    total_dx = float(px)
    total_dy = float(py)
    ...
    dx = total_dx / max(1, frame_gap)
    dy = total_dy / max(1, frame_gap)
    if math.hypot(dx, dy) < LARGE_FRONT_MIN_MOTION_MAG:
        return None
    return dx, dy, confidence


def consistent_large_front_animation_vector(vectors):
    """Find one direction supported across multiple times and multiple map tiles."""
    usable = []
    for dx, dy, confidence, pair_id, tile_id in vectors:
        mag = math.hypot(dx, dy)
        if mag < LARGE_FRONT_MIN_MOTION_MAG:
            continue
        if mag > LARGE_FRONT_MAX_SHIFT_PER_FRAME:
            continue
        angle = (math.degrees(math.atan2(dy, dx)) + 360.0) % 360.0
        usable.append((dx, dy, confidence, pair_id, tile_id, angle))

    if len(usable) < LARGE_FRONT_MIN_VALID_VECTORS:
        return None

    required = max(
        LARGE_FRONT_MIN_VALID_VECTORS,
        int(math.ceil(len(usable) * LARGE_FRONT_DIRECTION_CONSENSUS)),
    )
    ...
    for candidate in usable:
        cluster = [
            ...
            if angle_difference_deg(item[5], candidate[5])
            <= LARGE_FRONT_MAX_DIRECTION_DEVIATION_DEG
        ]
        if len(cluster) > len(best_cluster):
            best_cluster = cluster
        elif len(cluster) == len(best_cluster) and cluster:
            if sum(x[2] for x in cluster) > sum(x[2] for x in best_cluster):
                best_cluster = cluster

    if len(best_cluster) < required:
        return None

    vx = float(np.median([x[0] for x in best_cluster]))
    vy = float(np.median([x[1] for x in best_cluster]))
    if math.hypot(vx, vy) < LARGE_FRONT_MIN_MOTION_MAG:
        return None

    angle = (math.degrees(math.atan2(vy, vx)) + 360.0) % 360.0
    agreeing = [
        x for x in best_cluster
        if angle_difference_deg(x[5], angle)
        <= LARGE_FRONT_MAX_DIRECTION_DEVIATION_DEG
    ]
    if len(agreeing) < required:
        return None

    pair_count = len({x[3] for x in agreeing})
    tile_count = len({x[4] for x in agreeing})
    if pair_count < LARGE_FRONT_MIN_FRAME_PAIRS:
        return None
    if tile_count < LARGE_FRONT_MIN_SPATIAL_TILES:
        return None

    vx = float(np.median([x[0] for x in agreeing]))
    vy = float(np.median([x[1] for x in agreeing]))
    confidence = float(np.median([x[2] for x in agreeing]))
    consensus = len(agreeing) / max(1, len(usable))
    return vx, vy, len(agreeing), consensus, confidence, pair_count, tile_count, len(usable)


def estimate_large_front_animation_motion(
    frame_rgbs: List[np.ndarray],
    masks: List[np.ndarray],
    latest_component,
):
    """Final fallback for a huge unresolved front using SHMU RGB animation texture."""
    if (
        not LARGE_FRONT_ANIMATION_ENABLED
        or not frame_rgbs
        or not masks
        or len(frame_rgbs) != len(masks)
    ):
        return None

    area = int(latest_component["area"])
    map_share = area / max(1, WIDTH * MAP_BOTTOM)
    if area < LARGE_FRONT_MIN_PIXELS or map_share < LARGE_FRONT_MIN_MAP_SHARE:
        return None

    h, w = masks[-1].shape
    x0, y0, x1, y1 = latest_component["bbox"]
    x0 = max(0, x0 - LARGE_FRONT_ROI_MARGIN)
    y0 = max(0, y0 - LARGE_FRONT_ROI_MARGIN)
    x1 = min(w - 1, x1 + LARGE_FRONT_ROI_MARGIN)
    y1 = min(MAP_BOTTOM - 1, y1 + LARGE_FRONT_ROI_MARGIN)
    if x1 <= x0 or y1 <= y0:
        return None

    start = max(0, len(masks) - LARGE_FRONT_HISTORY_FRAMES)
    frame_indices = list(range(start, len(masks)))
    if len(frame_indices) < LARGE_FRONT_MIN_FRAME_PAIRS + 1:
        return None

    tile = LARGE_FRONT_TILE_SIZE
    step = LARGE_FRONT_TILE_STEP
    vectors = []

    # Keep the grid fixed in image coordinates. Overlapping tiles make the
    # estimate tolerant of local growth/decay while spatial-diversity gates
    # prevent one small patch from deciding the direction of an entire front.
    xs = list(range(x0, max(x0 + 1, x1 - tile + 2), step))
    ys = list(range(y0, max(y0 + 1, y1 - tile + 2), step))
    if not xs or xs[-1] + tile - 1 < x1:
        xs.append(max(x0, x1 - tile + 1))
    if not ys or ys[-1] + tile - 1 < y1:
        ys.append(max(y0, y1 - tile + 1))
    xs = sorted(set(max(0, min(w - tile, x)) for x in xs if w >= tile))
    ys = sorted(set(max(0, min(MAP_BOTTOM - tile, y)) for y in ys if MAP_BOTTOM >= tile))

    for a_idx, b_idx in zip(frame_indices[:-1], frame_indices[1:]):
        gap = b_idx - a_idx
        if gap <= 0:
            continue
        for ty in ys:
            for tx in xs:
                yy1 = min(ty + tile, MAP_BOTTOM)
                xx1 = min(tx + tile, w)
                a_mask = masks[a_idx][ty:yy1, tx:xx1]
                b_mask = masks[b_idx][ty:yy1, tx:xx1]
                if (
                    int(a_mask.sum()) < LARGE_FRONT_MIN_TILE_PIXELS
                    or int(b_mask.sum()) < LARGE_FRONT_MIN_TILE_PIXELS
                ):
                    continue
                shift = phase_shift_rgb_texture(
                    frame_rgbs[a_idx][ty:yy1, tx:xx1],
                    frame_rgbs[b_idx][ty:yy1, tx:xx1],
                    a_mask,
                    b_mask,
                    gap,
                )
                if shift is not None:
                    dx, dy, confidence = shift
                    vectors.append((dx, dy, confidence, (a_idx, b_idx), (tx, ty)))

    consensus_result = consistent_large_front_animation_vector(vectors)
    ...
    # Independent confirmations. Local RGB tiles are the primary signal; the
    # broad RGB baseline and existing binary front/centroid logic are only
    # cross-checks and do not replace any previous fallback algorithm.
    roi_rgbs = [rgb[y0:y1 + 1, x0:x1 + 1] for rgb in frame_rgbs]
    roi_masks = [mask[y0:y1 + 1, x0:x1 + 1] for mask in masks]
    confirmations = []
    global_rgb_vx = global_rgb_vy = None
    binary_long_vx = binary_long_vy = None
    centroid_vx = centroid_vy = None

    first_idx = frame_indices[0]
    last_idx = frame_indices[-1]
    long_gap = last_idx - first_idx
    if long_gap > 0:
        global_rgb = phase_shift_rgb_texture(
            roi_rgbs[first_idx],
            roi_rgbs[last_idx],
            roi_masks[first_idx],
            roi_masks[last_idx],
            long_gap,
            min_pixels=LARGE_FRONT_MIN_GLOBAL_PIXELS,
            min_confidence=LARGE_FRONT_MIN_PHASE_CONFIDENCE,
        )
        if global_rgb is not None:
            global_rgb_vx, global_rgb_vy, _ = global_rgb
            global_angle = (
                math.degrees(math.atan2(global_rgb_vy, global_rgb_vx)) + 360.0
            ) % 360.0
            diff = angle_difference_deg(angle, global_angle)
            if diff > LARGE_FRONT_HARD_CONTRADICTION_ANGLE_DEG:
                return None
            if diff <= LARGE_FRONT_CONFIRMATION_MAX_ANGLE_DEG:
                confirmations.append("rgb_long_baseline")

        first_binary = prepare_front_mask(roi_masks[first_idx])
        last_binary = prepare_front_mask(roi_masks[last_idx])
        binary_long = phase_shift_front(first_binary, last_binary, long_gap)
        if binary_long is not None:
            binary_long_vx, binary_long_vy, _ = binary_long
            binary_angle = (
                math.degrees(math.atan2(binary_long_vy, binary_long_vx)) + 360.0
            ) % 360.0
            diff = angle_difference_deg(angle, binary_angle)
            if diff > LARGE_FRONT_HARD_CONTRADICTION_ANGLE_DEG:
                return None
            if diff <= LARGE_FRONT_CONFIRMATION_MAX_ANGLE_DEG:
                confirmations.append("binary_long_baseline")

    indexed_masks = [
        (idx, prepare_front_mask(roi_masks[idx]))
        for idx in frame_indices
    ]
    centroid = front_centroid_vector(indexed_masks)
    if centroid is not None:
        centroid_vx, centroid_vy, _ = centroid
        centroid_angle = (
            math.degrees(math.atan2(centroid_vy, centroid_vx)) + 360.0
        ) % 360.0
        if angle_difference_deg(angle, centroid_angle) <= FRONT_FALLBACK_CENTROID_MAX_DEVIATION_DEG:
            confirmations.append("centroid_trend")
    ...
    return {
        ...
        "center": (latest_component["cx"], latest_component["cy"]),
        "area": area,
        "samples": int(agreeing_count),
        "motion_source": "large_front_animation",
        "animation_map_share": float(map_share),
        "animation_total_vectors": int(usable_count),
        "animation_agreeing_vectors": int(agreeing_count),
        "animation_direction_consensus": float(consensus),
        "animation_phase_confidence": float(phase_confidence),
        "animation_frame_pairs": int(pair_count),
        "animation_spatial_tiles": int(tile_count),
        "animation_global_rgb_vx": None if global_rgb_vx is None else float(global_rgb_vx),
        "animation_global_rgb_vy": None if global_rgb_vy is None else float(global_rgb_vy),
        "animation_binary_long_vx": None if binary_long_vx is None else float(binary_long_vx),
        "animation_binary_long_vy": None if binary_long_vy is None else float(binary_long_vy),
        "animation_centroid_vx": None if centroid_vx is None else float(centroid_vx),
        "animation_centroid_vy": None if centroid_vy is None else float(centroid_vy),
        "animation_confirmations": confirmations,
    }
...
def compute_component_motions(
    ...
    previous_info=None,
    frame_rgbs: Optional[List[np.ndarray]] = None,
):
    """
    ...
    if not candidates:
        return [], "no_component_over_threshold", 0, 0
    ...
    for component in candidates:
        ...
        motion, was_suppressed = apply_temporal_turn_guard(motion, previous_info)
        if was_suppressed:
            temporal_guard_suppressed += 1
    ...
    dominant = latest_components[0] if latest_components else None
    if FRONT_FALLBACK_ENABLED and dominant is not None:
        if id(dominant) not in resolved_component_ids:
            front_motion = estimate_front_fallback_motion(masks, dominant)
            front_motion, was_suppressed = apply_temporal_turn_guard(
                front_motion, previous_info
            )
            if was_suppressed:
                temporal_guard_suppressed += 1
    ...
    # FINAL fallback only: if every existing method left the dominant huge
    # system unresolved, inspect the internal RGB animation texture. This block
    # can never replace or modify an arrow produced by the pre-existing logic.
    large_front_animation_count = 0
    if (
        LARGE_FRONT_ANIMATION_ENABLED
        and dominant is not None
        and id(dominant) not in resolved_component_ids
        and frame_rgbs is not None
        and len(frame_rgbs) == len(masks)
    ):
        animation_motion = estimate_large_front_animation_motion(
            frame_rgbs,
            masks,
            dominant,
        )
        animation_motion, was_suppressed = apply_temporal_turn_guard(
            animation_motion, previous_info
        )
        if was_suppressed:
            temporal_guard_suppressed += 1
        if animation_motion is not None:
            motions.append(animation_motion)
            resolved_component_ids.add(id(dominant))
            large_front_animation_count = 1
    ...
    elif large_front_animation_count > 0:
        if len(motions) == 1:
            method = "large_front_animation_only"
        elif core_guided_count > 0:
            method = "per_component_core_guided_plus_large_front_animation"
        else:
            method = "per_component_phase_correlation_plus_large_front_animation"
    elif not motions:
    ...
    return motions, method, len(candidates), temporal_guard_suppressed
...
def main() -> None:
    ...
    _, display_hole_stats = trim_precip_for_display(
        latest_precip,
        latest_strong_core,
        return_stats=True,
    )

    previous_motion_info = load_previous_motion_info(latest_dt)
    (
        motions,
        motion_method,
        candidate_components,
        temporal_guard_suppressed,
    ) = compute_component_motions(
        precip_masks,
        strong_core_masks,
        previous_motion_info,
        frame_rgbs=frame_rgbs,
    )
    ...
    info = {
        ...
        "motion_precipitation_only_arrows": sum(
            1
            for motion in motions
            if motion.get("motion_source", "").startswith("precipitation")
        ),
        "motion_temporal_guard_suppressed": temporal_guard_suppressed,
        "motion_core_conflict_max_angle_deg": CORE_MOTION_CONFLICT_MAX_ANGLE_DEG,
        "motion_temporal_turn_max_angle_deg": TEMPORAL_TURN_MAX_ANGLE_DEG,
        "motion_front_fallback_arrows": sum(
            1
            for motion in motions
            if motion.get("motion_source") == "front_fallback"
        ),
        "motion_large_front_animation_arrows": sum(
            1
            for motion in motions
            if motion.get("motion_source") == "large_front_animation"
        ),
        ...
        "large_front_animation_enabled": LARGE_FRONT_ANIMATION_ENABLED,
        "large_front_animation_min_pixels": LARGE_FRONT_MIN_PIXELS,
        "large_front_animation_min_map_share": LARGE_FRONT_MIN_MAP_SHARE,
        "large_front_animation_history_frames": LARGE_FRONT_HISTORY_FRAMES,
        "display_hole_repair": {
            "enabled": DISPLAY_FILL_ENCLOSED_HOLES,
            "max_pixels": DISPLAY_HOLE_MAX_PIXELS,
            "max_width": DISPLAY_HOLE_MAX_WIDTH,
            "max_height": DISPLAY_HOLE_MAX_HEIGHT,
            **display_hole_stats,
        },
        ...
        info["movement_components"] = [
            {
                ...
                **(
                    ...
                    if "core_vx" in motion
                    else {}
                ),
                **(
                    {
                        "temporal_previous_direction": motion.get(
                            "temporal_previous_direction"
                        ),
                        "temporal_turn_deg": round(motion["temporal_turn_deg"], 1),
                        "temporal_match_distance_px": round(
                            motion["temporal_match_distance_px"], 1
                        ),
                        "temporal_guard_action": motion.get("temporal_guard_action"),
                    }
                    if "temporal_turn_deg" in motion
                    else {}
                ),
                **(
                    {
                ...
                **(
                    {
                        "animation_map_share": round(motion["animation_map_share"], 3),
                        "animation_total_vectors": motion["animation_total_vectors"],
                        "animation_agreeing_vectors": motion["animation_agreeing_vectors"],
                        "animation_direction_consensus": round(
                            motion["animation_direction_consensus"], 3
                        ),
                        "animation_phase_confidence": round(
                            motion["animation_phase_confidence"], 3
                        ),
                        "animation_frame_pairs": motion["animation_frame_pairs"],
                        "animation_spatial_tiles": motion["animation_spatial_tiles"],
                        "animation_global_rgb_vector_px_per_5min": (
                            None
                            if motion["animation_global_rgb_vx"] is None
                            else {
                                "vx": round(motion["animation_global_rgb_vx"], 2),
                                "vy": round(motion["animation_global_rgb_vy"], 2),
                            }
                        ),
                        "animation_binary_long_vector_px_per_5min": (
                            None
                            if motion["animation_binary_long_vx"] is None
                            else {
                                "vx": round(motion["animation_binary_long_vx"], 2),
                                "vy": round(motion["animation_binary_long_vy"], 2),
                            }
                        ),
                        "animation_centroid_vector_px_per_5min": (
                            None
                            if motion["animation_centroid_vx"] is None
                            else {
                                "vx": round(motion["animation_centroid_vx"], 2),
                                "vy": round(motion["animation_centroid_vy"], 2),
                            }
                        ),
                        "animation_confirmations": motion["animation_confirmations"],
                    }
                    if motion.get("motion_source") == "large_front_animation"
                    else {}
                ),
    ...
    print(
        ...
        f"large-front animation: "
        f"{sum(1 for m in motions if m.get('motion_source') == 'large_front_animation')}; "
        f"temporal suppressed: {temporal_guard_suppressed}"
    )
